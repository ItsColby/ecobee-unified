...
- One normalized immutable snapshot per mapping owns field selection,
  ...
  Beestat context, and recent command status. Source freshness uses Core's
...
- One climate entity per mapping projects the snapshot without property I/O and
  ...
  `device_entry` helper pattern. Registry reconciliation follows source-device
  moves, detachments, removals, and restorations, then schedules a supported
  config-entry reload without replacing stable identity.
...
| Correct Core 2026.8 device linkage | Proven locally | Real Core device/entity registry tests verify initial `device_entry`, move/detach/remove/restore relinking with supported reload, stable config/entity identity, and no foreign `device_info`. |
...
| Repository and HA workflows terminal green | Local subset green; hosted gate closed | Fifty local tests, strict mypy, Ruff, compile, actionlint, dependency closure, and a 38-file working-tree/tracked-archive plus complete reachable-history privacy scan pass. Linux pytest, Hassfest, and HACS jobs are defined but cannot be claimed terminal green before an authorized public repository run. |
...
`helper-device-linking` gap; its registry and product remediation remain owned
by the Home coordinator and Beestat, outside this repository.
...
- **Conditional shared pattern, existing capability:** a helper attached to a
  ...
  reload path without recreating the config entry; the Home coordinator owns
  portfolio promotion and Beestat disposition under `helper-device-linking`.
