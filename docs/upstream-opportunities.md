...
|---|---|---|
| Preserve the Ecobee HomeKit accessory's available current-temperature precision consistently across the HomeKit climate and same-service temperature-sensor projections, including updates. | Unified uses an explicitly mapped same-device sensor only while its valid, unit-normalized value agrees with the local climate's serialization envelope; after a confirmed disagreement, the precise source must also change from its last rejected value. This running-instance guard does not repair acquisition. | Undecided upstream opportunity; no Core change requested. |
