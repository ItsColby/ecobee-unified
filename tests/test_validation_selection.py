...
class ValidationSelectionTests(unittest.TestCase):
        ...
        for key, minimum, current_lane, release in (
            ...
            with (
                ...
                if key in {
                    ...
                    lane = key.removesuffix("_requirements")
                    self.assertTrue(plan["lane_tests"][lane])
                    self.assertTrue(plan["lane_typing"][lane])
                    self.assertIn("python -m mypy ", planner.lane_command(plan, lane))
                    other = "current" if lane == "minimum" else "minimum"
    ...
    def test_enum_translations_select_the_native_sensor_consumer(self):
        ...
        self.assertEqual(["tests/test_sensor.py"], plan["ha_tests"])
